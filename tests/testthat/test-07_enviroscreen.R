...
testthat::test_that("Recheck env_site_pctl by applying percentile to rechecked mean",{
  xwalk_ces_output <- xwalk_ces()
  xwalk_ces_output$pctl_recheck = percent_rank(rowMeans(select(xwalk_ces_output,cleanup_sites,hazard_waste,groundwater,solid_waste),na.rm=TRUE))
  testthat::expect_equal(xwalk_ces_output$env_site_pctl,xwalk_ces_output$pctl_recheck)
})
