...
# rural areas filter
# 11/21/23 - added county-level data on race/ethnicity to final shapefile
# 12/20/23 - incorporated post-comments edits: remove high college/grad
# population tracts, baseline year of 2013 for rent increase, reduce rent
# threshold to 50th percentile, include home value/income percentile gap
# (25 pp in 2021),include Part 3 pathway (75th percentile demo change, 2013-2021)
...
# Directories:
homedir <- paste0(here::here(),'/R/neighborhood_change/2024')
workdir <- "/Data/"
...
tract2020_df <-
  read_csv(paste0(homedir, "/Results/tract2020_11212023.csv"))
...
college_df <-
  ...
  select(tract2020 = GEOID, collegepop21 = estimate) %>%
  left_join(tract2020_df %>% select(tract2020, trct_pop_total_2021)) %>%
  transmute(
    tract = tract2020,
    college_flag = if_else(collegepop21 / trct_pop_total_2021 >= .25, 1, 0)
  )
...
rm(army_df, inc_cv, race_cv, raceeth21_df, income21_df, pop_density_df,
   prisoner_df, totalpop_df, college_df)
...
raceeth_df <-
  ...
  mutate(
    ...
    # repeat above for 2013-2021 period (50th and 75th pct)
    ...
    raceeth_quarter1321 = quantile(temp, probs = .75, na.rm = TRUE),
    raceeth_quarter1321 = replace_na(raceeth_quarter1321, 0)
  ) %>%
  ungroup() %>%
  select(-temp) %>%
  # generate flags
  mutate(
    ...
    change_race0021 =
      if_else(
        baseline_raceinc0021 == 1 &trct_raceeth_chng0021 >= raceeth_half0021,
        1, 0
      ),
    ...
    ## condition 2b
    change_race1321_75pct =
      if_else(
        baseline_race1321 == 1 & trct_raceeth_chng1321 >= raceeth_quarter1321,
        1, 0
      )
  ) %>%
  select(
    ...
    raceeth_half0021, raceeth_half1321, raceeth_quarter1321,
...
income_df <-
  ...
  mutate(
    ...
    inc_quarter1321 = quantile(temp, probs = .75, na.rm = TRUE),
    inc_quarter1321 = replace_na(inc_quarter1321, 0)
  ) %>%
  ungroup() %>%
  select(-temp) %>%
  mutate(
    ...
    ## condition 2b
    change_income1321_75pct =
      if_else(
        baseline_income1321 == 1 & trct_inc_chng1321 >= inc_quarter1321, 1, 0
      )
  ) %>%
  select(
    # descriptive data
    ...
    cnty_inc_chng1321,
    # countywide thresholds
    inc_half0021, inc_half1321, inc_quarter1321,
    # baseline flags (2000, 2013)
    starts_with("baseline"),
    # change flags (2000, 2013)
    starts_with("change")
...
rent_df <-
  ...
  mutate(
    ...
    # 2013-2021 (50%)
    rent_half1321 = quantile(temp, probs = .50, na.rm = TRUE),
    rent_half1321 = replace_na(rent_half1321, 0)
  ) %>%
  ungroup() %>%
  select(-temp) %>%
  ...
  left_join(
    income_df %>%
      ...
      select(tract2020, trct_medinc_2013, cnty_medinc_2013, baseline_income1321)
  ) %>%
  mutate(
    ...
    medrent_disp1321 =
      if_else(
        ...
        trct_medinc_2013 <= (cnty_medinc_2013 * 1.2) & rural_flag == 0 &
          # Condition 2 (change)
          # trct_pctchng_medrent1321 >= rent_quarter1321,
          trct_pctchng_medrent1321 >= rent_half1321,
        1, 0
      )
  ) %>%
  select(
...
## Percentile rank of tract median income (compared to county)
inc_pct <-
  tract2020_df %>%
  filter(!is.na(county)) %>%
  select(
    tract2020, county, contains("trct_medinc"), contains("trct_pop_total")
  ) %>%
  pivot_longer(
  ...
  select(-year_copy) %>%
  filter(!c(county %in% c("003", "091"))) %>%
  group_by(county, year) %>%
  arrange(county, year, medinc) %>%
  mutate(
    income_percentile =
      if_else(
        !is.na(medinc),
...
hval_pct <-
  ...
  filter(!is.na(county)) %>%
  select(
    tract2020, county, contains("trct_medhval"), contains("trct_ownhu")
  ) %>%
  pivot_longer(
...
## meets Part 1 and meets Criteria 3 (rapidly rising rents or home value/income gap),
...
nbrhood_chng_df <-
      ...
      if_else(
        ...
        (halfmile_buffer == 1 &
           ...
           #    (change_race1321 == 1 | change_income1321 == 1)
            ((medrent_disp1321 == 1 | hvalinc_gap == 1) &
                (change_race1321 == 1 | change_income1321 == 1)
            )
        ) & rural_flag == 0,
        1, 0
  ...
  #       cnty_medrent_2021, trct_medrent_2013, cnty_medrent_2013,
  #       matches("trct_pct"),
  #       matches("trct_pop_\\w+_2000"), matches("trct_pop_\\w+_2013"),
  #       matches("trct_pop_\\w+_2021"), matches("trct_pct_\\w+_2000"),
  #       matches("trct_pct_\\w+_2013"), matches("trct_pct_\\w+_2021"),
  #       matches("cnty_pop_\\w+_2000"), matches("cnty_pop_\\w+_2013"),
  #       matches("cnty_pop_\\w+_2021"), matches("cnty_pct_\\w+_2000"),
  #       matches("cnty_pct_\\w+_2013"), matches("cnty_pct_\\w+_2021"),
...
## Part 3 (Criteria 1 and 2, 2013-2021) -----------------------------------
nbrhood_chng_df <-
  nbrhood_chng_df %>%
  mutate(
    part3 =
      if_else(
        (baseline_race1321 == 1 & baseline_income1321 == 1) &
          (trct_raceeth_chng1321 >= raceeth_quarter1321 &
             trct_inc_chng1321 >= inc_quarter1321) &
          rural_flag == 0,
        1, 0
      ),
    part3 = if_else(exclusion_flag == 1, NA_real_, part3),

    # create neighborhood change definition flag
    nbrhood_chng = if_else(part1 == 1 | part2 == 1 | part3 == 1, 1, 0)
  ) %>%
  # join back with other useful demographic data
  left_join(
    tract2020_df %>%
...
nbrhood_chng_df %>%
  st_drop_geometry() %>%
  write_csv(
    paste0(homedir, savedir, "neighborhood_change_layer_",
           #todays_date,
           '2024',
           ".csv")
  )
